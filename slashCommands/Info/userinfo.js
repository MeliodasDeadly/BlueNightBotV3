...
module.exports = {
  ...
  options: [ //OPTIONAL OPTIONS, make the array empty / dont add this option if you don't need options!	
	//INFORMATIONS! You can add Options, but mind that the NAME MUST BE LOWERCASED! AND NO SPACES!!!, for the CHOCIES you need to add a array of arrays; [ ["",""] , ["",""] ] 
		...
		{"User": { name: "user", description: "From Which User do you want to get Information from?", required: true }}, //to use in the code: interacton.getUser("ping_a_user")
  ...
  run: async (client, interaction) => {
    try{
		...
		try{
			...
			embeduserinfo.addFields('**❱ Username:**',`<@${member.user.id}>\n\`${member.user.tag}\``,true)
			embeduserinfo.addFields('**❱ ID:**',`\`${member.id}\``,true)
			embeduserinfo.addFields('**❱ Avatar:**',`[\`Link to avatar\`](${member.user.displayAvatarURL({ format: "png" })})`,true)
			embeduserinfo.addFields('**❱ Date Join DC:**', "\`"+moment(member.user.createdTimestamp).format("DD/MM/YYYY") + "\`\n" + "`"+ moment(member.user.createdTimestamp).format("hh:mm:ss") + "\`",true)
			embeduserinfo.addFields('**❱ Date Join Guild:**', "\`"+moment(member.joinedTimestamp).format("DD/MM/YYYY") + "\`\n" + "`"+ moment(member.joinedTimestamp).format("hh:mm:ss")+ "\`",true)
			embeduserinfo.addFields('**❱ Flags:**',`\`${userFlags.length ? userFlags.map(flag => flags[flag]).join(', ') : 'None'}\``,true)
			embeduserinfo.addFields('**❱ Status:**',`\`${statuses[member.user.presence?.status]} ${member.user.presence?.status}\``,true)
			embeduserinfo.addFields('**❱ Highest Role:**',`${member.roles.highest.id === guild.id ? 'None' : member.roles.highest}`,true)
			embeduserinfo.addFields('**❱ Is a Bot:**',`\`${member.user.bot ? "✔️" : "❌"}\``,true)
