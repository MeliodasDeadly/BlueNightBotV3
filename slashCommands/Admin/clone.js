...
module.exports = {
  name: "clone", //the command name for the Slash Command
  description: "Clone a channel", //the command description for Slash Command Overview
  cooldown: 5,
  ...
  run: async (client, interaction) => {
    ...
    if (!ChannelOption) ChannelOption = interaction.channel;
       ...
       // get interaction.getChannel("what_channel") and clone it
        channel.clone().then((ch) => {
            interaction.reply({content: `Successfully cloned ${channel}`, ephemeral: true});
        }).catch((err) => {})
